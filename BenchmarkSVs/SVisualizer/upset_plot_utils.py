...
def make_disqualified_df(close_df, dist_threshold, size_ratio_threshold, color=None):
    ...
    disq_df['DIST'] = close_df['DIST'].apply(lambda x: 1 if np.abs(x) > dist_threshold else 0)
    disq_df['MATCH'] = 1 - (close_df['LTYPE'] == close_df['RTYPE']).astype(int)
    disq_df['FILTER'] = close_df['RFILTER'].apply(lambda x: 1 if x != 'PASS' and x != '.' else 0)
    disq_df['SIZE'] = close_df['SIZE_RATIO'].apply(lambda x: 1 if x < size_ratio_threshold else 0)
    if color is not None:
        disq_df[color] = close_df[color].astype(str)
    return disq_df
