...
def get_sc_metrics(sc_metrics_path, sample_id):
    ...
    calculated_rows.append({'metric': 'Total Gene Input Reads', 'value': metrics.loc[metrics['metric'] == 'Total barcoded reads', 'value'].values[0] + metrics.loc[metrics['metric'] == 'Reads with non-matching barcodes', 'value'].values[0] + metrics.loc[metrics['metric'] == 'Reads missing barcodes', 'value'].values[0]})
    calculated_rows.append({'metric': 'Fraction Reads with Valid Barcodes', 'value_frac': metrics.loc[metrics['metric'] == 'Total barcoded reads', 'value'].values[0] / (metrics.loc[metrics['metric'] == 'Total barcoded reads', 'value'].values[0] + metrics.loc[metrics['metric'] == 'Reads with non-matching barcodes', 'value'].values[0] + metrics.loc[metrics['metric'] == 'Reads missing barcodes', 'value'].values[0])})
    calculated_rows.append({'metric': 'Fraction Reads with valid IMI', 'value_frac': (metrics.loc[metrics['metric'] == 'Reads with valid molecular identifier sequences', 'value'].values[0] + metrics.loc[metrics['metric'] == 'Reads with corrected molecular identifier sequences', 'value'].values[0]) / metrics.loc[metrics['metric'] == 'Total barcoded reads', 'value'].values[0]})
    calculated_rows.append({'metric': 'Fraction Mapped Reads', 'value_frac': (metrics.loc[metrics['metric'] == 'Unique exon matching reads', 'value'].values[0] + metrics.loc[metrics['metric'] == 'Unique intron matching reads', 'value'].values[0] + metrics.loc[metrics['metric'] == 'Mitochondrial reads', 'value'].values[0]) / metrics.loc[metrics['metric'] == 'Total barcoded reads', 'value'].values[0]})
    calculated_rows.append({'metric': 'Mean reads per cell', 'value': metrics.loc[metrics['metric'] == 'Total gene reads', 'value'].values[0] / metrics.loc[metrics['metric'] == 'Passing cells', 'value'].values[0]})
    calculated_rows.append({'metric': 'Fraction CRISPR Barcoded Reads', 'value_frac': metrics.loc[metrics['metric'] == 'Total CRISPR reads matching known barcodes', 'value'].values[0] / metrics.loc[metrics['metric'] == 'Total feature reads', 'value'].values[0]})
