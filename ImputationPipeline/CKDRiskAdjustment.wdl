...
task AdjustRisk {
  ...
  command <<<
    Rscript -<< "EOF"
      ...
      adjusted_scores <- adjusted_scores %>% mutate(ckd_adjusted_score = adjusted_score + apol1_high_risk)
      adjusted_scores <- adjusted_scores %>% mutate(ckd_percentile = pnorm(adjusted_score))
