...
task BcftoolsCall {
    ...
    command <<<
        ...
        bcftools mpileup -f ~{fasta} ~{if !call_indels then "-I" else ""} -G sample_name_mapping.txt -E -a 'FORMAT/DP,FORMAT/AD' -T ~{sites_vcf} -Ou ~{sep=" " crams} | bcftools call -Aim -C alleles -T sites.tsv.gz -Ou | bcftools norm -m -both -Oz -o ~{out_basename}.vcf.gz
