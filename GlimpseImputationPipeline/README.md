...
- [Glimpse2ImputationAndCheckQC](Glimpse2ImputationAndCheckQC.wdl): Runs Glimpse2Imputation and afterwards Glimpse2CheckQC. For inputs and outputs, please refer to the documentation for these two workflows. ([interactive diagram](https://raw.githack.com/broadinstitute/palantir-workflows/main/docs/viz/GlimpseImputationPipeline/Glimpse2ImputationAndCheckQC.html) · [open locally](../docs/viz/GlimpseImputationPipeline/Glimpse2ImputationAndCheckQC.html))
- [Glimpse2QCReport](#Glimpse2QCReport): Generate a PDF QC report from an imputed VCF and the QC metrics generated by Glimpse2Imputation
- [ReduceAndMergeForGlimpse](ReduceAndMergeForGlimpse.wdl): Remove all annotations except GT and PL from single-sample VCFs and merge them into a multi-sample VCF ([interactive diagram](https://raw.githack.com/broadinstitute/palantir-workflows/main/docs/viz/GlimpseImputationPipeline/ReduceAndMergeForGlimpse.html) · [open locally](../docs/viz/GlimpseImputationPipeline/ReduceAndMergeForGlimpse.html))
- [Glimpse1Imputation](Glimpse1Imputation.wdl): Run GLIMPSE1 imputation (not well-tested or optimized) ([interactive diagram](https://raw.githack.com/broadinstitute/palantir-workflows/main/docs/viz/GlimpseImputationPipeline/Glimpse1Imputation.html) · [open locally](../docs/viz/GlimpseImputationPipeline/Glimpse1Imputation.html))
...
**Interactive diagram:** [view on GitHub](https://raw.githack.com/broadinstitute/palantir-workflows/main/docs/viz/GlimpseImputationPipeline/Glimpse2SplitReference.html) · [open locally](../docs/viz/GlimpseImputationPipeline/Glimpse2SplitReference.html)
...
**Interactive diagram:** [view on GitHub](https://raw.githack.com/broadinstitute/palantir-workflows/main/docs/viz/GlimpseImputationPipeline/Glimpse2Imputation.html) · [open locally](../docs/viz/GlimpseImputationPipeline/Glimpse2Imputation.html)
...
**Interactive diagram:** [view on GitHub](https://raw.githack.com/broadinstitute/palantir-workflows/main/docs/viz/GlimpseImputationPipeline/Glimpse2MergeBatches.html) · [open locally](../docs/viz/GlimpseImputationPipeline/Glimpse2MergeBatches.html)
...
**Interactive diagram:** [view on GitHub](https://raw.githack.com/broadinstitute/palantir-workflows/main/docs/viz/GlimpseImputationPipeline/Glimpse2ImputationInBatches.html) · [open locally](../docs/viz/GlimpseImputationPipeline/Glimpse2ImputationInBatches.html)
...
**Interactive diagram:** [view on GitHub](https://raw.githack.com/broadinstitute/palantir-workflows/main/docs/viz/GlimpseImputationPipeline/Glimpse2CheckQC.html) · [open locally](../docs/viz/GlimpseImputationPipeline/Glimpse2CheckQC.html)
...
This workflow generates a PDF QC report for an imputed cohort. It combines the QC metrics and coverage metrics produced during imputation with information extracted from the imputed VCF itself. Note that this workflow is independent of [Glimpse2CheckQC](#Glimpse2CheckQC): it is not called by any other workflow in this directory and does not check any metric against thresholds, it only summarizes and visualizes them. It is also not run automatically as part of [Glimpse2Imputation](#Glimpse2Imputation), and has to be launched separately on that workflow's outputs.

The workflow first estimates the genetic ancestry of each sample by extracting dosages at the sites listed in `pca_loadings` using `bcftools +dosage`, projecting them onto the principal components defined by those loadings, and classifying the resulting principal components with the ONNX model provided in `onnx_model`. The report describes these resources as being provided by [gnomAD](https://gnomad.broadinstitute.org/downloads#v3-ancestry-classification) and notes that the resulting ancestry estimates are used solely for QC. In parallel, the `RAF` and `INFO` annotations are extracted from the imputed VCF using `bcftools query`. Finally, an R Markdown document is assembled from these intermediate files and rendered to PDF.

The resulting report consists of a site QC section, showing the distributions of mean coverage and of the fraction of sites covered by at least one read (both per sample × chunk and averaged per sample), the number and fraction of sites with an INFO score above 0.6 and 0.8 for several reference panel allele frequency thresholds, and the relationship between reference panel allele frequency and INFO score; and a sample QC section, showing the ancestry PCA scatter plot with per-ancestry sample counts, and histograms of the Hail [`sample_qc`](https://hail.is/docs/0.2/methods/genetics.html#hail.methods.sample_qc) metrics stratified by estimated ancestry.

**Interactive diagram:** [view on GitHub](https://raw.githack.com/broadinstitute/palantir-workflows/main/docs/viz/GlimpseImputationPipeline/Glimpse2QCReport.html) · [open locally](../docs/viz/GlimpseImputationPipeline/Glimpse2QCReport.html)

### Input
- **String cohort_name**: Name of the cohort. Used as the subtitle of the report and as the basename for the output file.
- **File metrics**: QC metrics produced by [Glimpse2Imputation](#Glimpse2Imputation) (`qc_metrics`) or [Glimpse2MergeBatches](#Glimpse2MergeBatches) (`merged_qc_metrics`). Must contain a `sample_id` column, which is used to join the metrics with the estimated ancestries.
- **File vcf**: The imputed VCF, used for ancestry estimation and for extracting the `RAF` and `INFO` annotations.
- **File vcf_index**: Index to `vcf`.
- **File pca_loadings**: TSV file (with header) of PCA loadings, containing the columns `contig`, `pos`, `ref`, `alt`, `pca_af`, and one column per principal component named `pc1`, `pc2`, ....
- **File onnx_model**: ONNX model that predicts an ancestry label from the principal components defined by `pca_loadings`.
- **File coverage_metrics**: The `coverage_metrics` output of [Glimpse2Imputation](#Glimpse2Imputation), which is only produced when running imputation on CRAM input. Must contain the columns `Sample`, `Chunk`, `Cov.`, and `No data pct`.

### Output
- **File qc_report**: PDF QC report, named `{cohort_name}_qc_report.pdf`.
