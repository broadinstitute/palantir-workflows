...
workflow GenerateDuplexConsensusBams {
   ...
   call CollectSequencingArtifactMetrics as BeforeUmiDeduplicationArtifactMetrics {
      input:
         bloodbiopsydocker=bloodbiopsydocker,
         bam_file = preprocessed_raw_bam,
         bam_index = preprocessed_raw_bam_index,
   ...
   # Collect Sequencing Artifact Metrics after deduplication by start and stop
   # positions (but not including UMIs).
   call CollectSequencingArtifactMetrics as ConsensusArtifactMetrics {
      input:
         bloodbiopsydocker=bloodbiopsydocker,
...
task AlignRawBamWithBwaMem {
   ...
   File ref_sa
   Int compression_level
   String? revert_sam_extra_args
   String? merge_bam_alignments_extra_args
   Int? preemptible_attempts
   Int? memory
   Int mem = select_first([memory, 15])
   Int disk_pad
...
task CollectSequencingArtifactMetrics {
   ...
   command {
      ...
      java -Xmx${compute_mem}m -jar $PICARD_LOCAL_JAR \
         ...
         I=${bam_file} O="${base_name}" \
         R=${reference} \
         VALIDATION_STRINGENCY=LENIENT

      # Convert to GATK format for use later in FilterByOrientationBias
      sed -r "s/picard\.analysis\.artifacts\.SequencingArtifactMetrics\\\$PreAdapterDetailMetrics/org\.broadinstitute\.hellbender\.tools\.picard\.analysis\.artifacts\.SequencingArtifactMetrics\$PreAdapterDetailMetrics/g" "${base_name}.pre_adapter_detail_metrics" > "${base_name}.gatk.pre_adapter_detail_metrics"
   ...
   output {
      File gatk_pre_adapter_detail_metrics = "${base_name}.gatk.pre_adapter_detail_metrics"
