...
`FunctionalEquivalence.wdl` is the only entry point of this pipeline.

**Interactive diagram:** [view on GitHub](https://raw.githack.com/broadinstitute/palantir-workflows/main/docs/viz/FunctionalEquivalence/FunctionalEquivalence.html) · [open locally](../docs/viz/FunctionalEquivalence/FunctionalEquivalence.html)

It runs all of the pairwise comparisons by calling the `BenchmarkVCFs` workflow from the neighboring [BenchmarkVCFs](../BenchmarkVCFs) directory, and then delegates the aggregation and plotting of those comparisons to the three files in the `subworkflows` directory. These are internal to the pipeline and are not meant to be run directly. Note that despite the directory name, [`FEEvaluation.wdl`](subworkflows/FEEvaluation.wdl) and [`F1Evaluation.wdl`](subworkflows/F1Evaluation.wdl) each contain a single WDL task rather than a workflow, while [`PlotROC.wdl`](subworkflows/PlotROC.wdl) wraps its task in a `PlotROC` workflow.

- [`subworkflows/FEEvaluation.wdl`](subworkflows/FEEvaluation.wdl): Called once, as `RunFEEvaluation`, on the summary tables of the inter tool and both intra tool comparisons. It computes the Jaccard score for each comparison, aggregates the mean and standard error per dataset, variant type and stratifier interval, and produces the FE plots, the `fe_summary` table, and an `fe_status` indicator that is reflected in the background color of each plot title.
- [`subworkflows/F1Evaluation.wdl`](subworkflows/F1Evaluation.wdl): Called once, as `RunF1Evaluation`, on the ROC tables of the comparisons against the truth VCFs. It computes the F1 score as a function of the quality threshold, takes the differences between replicates of the same tool (intra) and between the two tools on the same replicate (inter), and produces the F1 plots, the `f1_summary` table, and an `fe_status` indicator. The workflow-level `fe_status` output is the larger of the values returned by this task and by `FEEvaluation`.
- [`subworkflows/PlotROC.wdl`](subworkflows/PlotROC.wdl): Called in a scatter over samples, as `PlotROCTask`, on the pair of ROC tables produced for that sample by the comparisons of tool 1 and tool 2 against the truth VCF. It plots sensitivity against the number of false positives for both tools and annotates the quality threshold at which each tool reaches its maximum F1 score. Its plots make up the `roc_plots` and `merged_roc_plots` outputs.
